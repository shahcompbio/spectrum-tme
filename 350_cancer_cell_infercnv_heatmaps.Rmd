...
inferCNV_clone_tbl <- lapply(inferCNV_clone_files, read_csv) %>% 
  ...
  mutate(clone_label = ifelse(clone_label == "observation.1.1", "observation.1.1.1", clone_label))

inferCNV_obj_list <- lapply(inferCNV_obj_files, read_rds)

n_clones <- length(unique(inferCNV_clone_tbl$clone_label))

clrs$clone_label <- magma(n_clones+1)[-c(1)]
...
anno_data <- seu_obj_cc %>% 
  ...
  arrange(consensus_signature, patient_id_short, clone_label, tumor_supersite, cluster_label) %>% 
  mutate(cell_id_order_clone2 = ordered(cell_id, levels = as.character(cell_id))) %>% 
...
plot_data <- as.data.frame(expr_cc) %>% 
  ...
  left_join(select(anno_data, cell_id, cell_id_order_clone, cell_id_order_cluster, cell_id_order_cohort, cell_id_order_clone2, patient_id_short, consensus_signature), 
            by = "cell_id") %>% 
...
plot_inferCNV_heat_wrapper <- function(pids, group_label = paste0(pids, collapse = "_"), cell_id_order, downsample = F, downsample_var = sample_id, yscaling = 2000, return_data = F) {
  
  cell_id_order <- enquo(cell_id_order)
  downsample_var <- enquo(downsample_var)
  plot_data_sub <- filter(plot_data, patient_id_short %in% pids)
  # plot_data_sub <- filter(plot_data_sub, chr %in% c(" chr20"))
  ...
  if (downsample != F) {
    ...
    ncells <- unique(length(anno_data_sub$cell_id))
    plotheight <- ncells/downsample/(yscaling/500)
  ...
  if (!return_data) {
    ...
    if (as_label(cell_id_order) == "cell_id_order_clone") {
      ...
      pg <- plot_grid(plot_dendro, plot_anno_patient, plot_anno_mutsig, plot_anno_site, plot_anno_clone, 
                      # plot_anno_6pbaf, 
                      plot_anno_cluster, plot_anno_jakstat, plot_anno_nfkb, plot_anno_tnfa, plot_anno_tgfb, plot_anno_hypoxia, ggdraw(), plot_cnv+remove_guides, nrow = 1, align = "h", axis = "tb", rel_widths = c(0.1, rep(0.02, 10), 0.005, 1-0.1-10*0.02-0.005))
      
    } else {
      
      pg <- plot_grid(ggdraw(), plot_anno_patient, plot_anno_mutsig, plot_anno_site, plot_anno_clone, 
                      # plot_anno_6pbaf, 
                      plot_anno_cluster, plot_anno_jakstat, plot_anno_nfkb, plot_anno_tnfa, plot_anno_tgfb, plot_anno_hypoxia, ggdraw(), plot_cnv+remove_guides, nrow = 1, align = "h", axis = "tb", rel_widths = c(0.1, rep(0.02, 10), 0.005, 1-0.1-10*0.02-0.005))
  ...
  result <- paste0(group_label, " DONE")
  
  if (return_data) result <- list(plot_data = plot_data_sub, anno_data = anno_data_sub)
  
  return(result)
  
}

plot_inferCNV_heat_wrapper(c("007", "036", "107"), "pois_cluster", cell_id_order_cluster, yscaling = 4000)
plot_inferCNV_heat_wrapper(c("007", "036", "107"), "pois_clone", cell_id_order_clone2, yscaling = 4000)
# plot_inferCNV_heat_wrapper(c("007", "036", "107"), "pois_clone_dendro", cell_id_order_clone, yscaling = 4000)
...
plot_inferCNV_heat_wrapper(sort(unique(anno_data$patient_id_short)), "cohort_patients_balanced_clonesort", cell_id_order_clone2, 50, patient_id_short)
plot_inferCNV_heat_wrapper(sort(unique(anno_data$patient_id_short)), "cohort_patients_balanced_clustersort", cell_id_order_cluster, 50, patient_id_short)
...
```{r chunk_350_080, fig.width=7, fig.height=20}

cohort_data <- plot_inferCNV_heat_wrapper(sort(unique(anno_data$patient_id_short)), "cohort_patients_balanced_clonesort", cell_id_order_clone2, 50, patient_id_short, return_data = T)

cohort_data_sum <- cohort_data$plot_data %>% 
  mutate(value_scaled = (value_cut - 1)*4) %>% 
  group_by(gene, chr, rank, consensus_signature) %>% 
  mutate() %>% 
  summarise(n_cells = length(value_scaled),
            value_sum = sum(value_scaled)) %>% 
  mutate(value_rel = value_sum/n_cells) %>% 
  arrange(-value_rel, -value_sum) %>% 
  ungroup() %>% 
  mutate(updown = ifelse(value_rel > 0, "up", "down"))

cohort_data_sum %>% 
  filter(value_rel > 0.5 | value_rel < 0.5)

```

```{r chunk_350_090, fig.width=7, fig.height=20}

ggplot(cohort_data_sum) +
  geom_bar(aes(rank, value_rel, fill = updown), stat = "identity") +
  scale_fill_manual(values = c(up = "#fc8d59", down = "#3182bd")) +
  facet_grid(consensus_signature~chr, scales = "free_x", space = "free_x") +
  labs(y = "Fraction of patients\nwith AMP or DEL [%]") +
  remove_xaxis +
  remove_guides
