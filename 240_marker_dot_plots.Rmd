...
load_super_set <- function(super_set, cluster_var) {
  ...
  marker_mat <- read_tsv(paste0("/work/shah/users/uhlitzf/data/SPECTRUM/freeze/v7/supplementary_tables/", super_set, "_marker_sheet.tsv")) %>% 
    slice(1:20)
  ...
  if (as_label(cluster_var) == "cluster_label_sub") {
    ...
    marker_mat <- read_tsv(paste0("/work/shah/users/uhlitzf/data/SPECTRUM/freeze/v7/supplementary_tables/", super_set, "_marker_sheet_full.tsv")) %>% 
      slice(1:20)
  ...
  marker_list <- marker_mat %>% 
    ...
    .[names(.)[names(.) %in% unique(unlist(seu_obj[[as_label(cluster_var)]]))]]
  
  defining_genes <- list()
  for (i in 1:length(marker_list)) {
    defining_genes[[i]] <- setdiff(
      marker_list[[i]], 
      unique(unlist(marker_list[-i]))
    )
  }
  
  defining_genes <- setNames(defining_genes, names(marker_list)) %>% 
    enframe("cluster_marker", "gene") %>% 
    unnest(gene) %>% 
    group_by(cluster_marker) %>% 
    slice(1:2) %>% 
    ungroup()
  
  top_marker_tbl <- marker_tbl_full %>% 
  ...
  marker_tbl <- bind_rows(defining_genes, top_marker_tbl) %>% 
    distinct(cluster_marker, gene) %>% 
    group_by(cluster_marker) %>% 
    slice(1:5) %>% 
    ungroup() %>% 
...
dot_plot_data_tcell <- marker_dot_plot_preprocess(dot_plot_input_tcell$seu_obj, dot_plot_input_tcell$marker_tbl, cluster_label, canonical_genes = c("CD4", "FOXP3", "CD8A", "CD8B", "GNLY"))
...
dot_plot_data_tcell_sub <- marker_dot_plot_preprocess(dot_plot_input_tcell_sub$seu_obj, dot_plot_input_tcell_sub$marker_tbl, cluster_label_sub, top_n = 3, canonical_genes = c("CD4", "FOXP3", "CD8A", "CD8B", "GNLY"))
