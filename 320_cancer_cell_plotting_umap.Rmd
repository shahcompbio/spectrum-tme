...
plot_data_summary_site <- plot_data_pw %>%
  group_by(cluster_label, tumor_supersite, pathway) %>%
  pathway_summary_wrapper %>% 
  mutate(cluster_label = ordered(cluster_label, levels = str_remove_all(names(clrs$cluster_label$Ovarian.cancer.super), "\\.cell")))
...
```{r chunk_320_160, fig.width=8, fig.height=6.5}

major_sites <- c("Adnexa", "Ascites", "Omentum", "Bowel")
...
pw_breaks <- c(seq(min(pw_mean_score), 0.5, length.out = 5)[-5], 
               0.5, seq(0.5, max(pw_mean_score), length.out = 5)[-1])
...
pw_plot1 <- plot_data_summary_site %>% 
  ...
  filter(tumor_supersite %in% major_sites) %>% 
  ggplot() +
  geom_tile(aes(pathway, cluster_label, fill = mean_score)) +
  common_heat_layers +
  facet_wrap(~tumor_supersite, ncol = 2) + 
...
pw_plot1_anno <- plot_data_summary_site %>% 
  ...
  filter(tumor_supersite %in% major_sites[1:2]) %>% 
  mutate(tumor_supersite = case_when(
    tumor_supersite == "Adnexa" ~ " ",
    tumor_supersite == "Ascites" ~ "  ",
    T ~ " "
  )) %>% 
  distinct(cluster_label, tumor_supersite, .keep_all = T) %>% 
  ggplot() +
  geom_tile(aes(tumor_supersite, cluster_label, fill = cluster_label)) +
  geom_point(aes(tumor_supersite, cluster_label), color = "white", alpha = 0.5, size = 5) +
  geom_text(aes(tumor_supersite, cluster_label, label = as.numeric(cluster_label))) +
  common_heat_layers + 
  scale_fill_manual(values = clrs$cluster_label$Ovarian.cancer.super %>% setNames(str_remove_all(names(.), "\\.cell"))) +
  facet_wrap(~tumor_supersite, ncol = 1, scales = "free_x") +
  theme(axis.text.x = element_blank(),
        axis.text.y = element_text()) +
  guides(fill = F)

pw_heat_grid <- plot_grid(pw_plot1_anno, pw_plot1 + remove_guides, ncol = 2, align = "h", axis = "tb", rel_widths = c(0.22, 0.78))

ggdraw() +
  draw_plot(pw_heat_grid, width = 1, height = 0.85, x = 0, y = 0.15) +
  draw_plot(get_legend(pw_plot1), x = -0.4, y = -0.35)

ggsave("figures/320_cancer_cell_plotting_umap/003_pathway_heatmap.pdf", width = 8, height = 6.5)
ggsave("figures/320_cancer_cell_plotting_umap/003_pathway_heatmap.png", width = 8, height = 6.5)
...
```{r chunk_320_170, fig.width=6, fig.height=6.5}
...
hla_heat_grid <- plot_grid(pw_plot1_anno, hla_plot1 + remove_guides, ncol = 2, align = "h", axis = "tb", rel_widths = c(0.4, 0.6))

ggdraw() +
  draw_plot(hla_heat_grid, width = 1, height = 0.85, x = 0, y = 0.15) +
  draw_plot(get_legend(hla_plot1), x = -0.3, y = -0.28)

ggsave("figures/320_cancer_cell_plotting_umap/003_hla_heatmap.pdf", width = 4.5, height = 6.5)
ggsave("figures/320_cancer_cell_plotting_umap/003_hla_heatmap.png", width = 4.5, height = 6.5)
