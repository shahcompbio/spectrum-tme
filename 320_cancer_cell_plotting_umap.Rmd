...
```{r chunk_320_073, fig.width=6.5, fig.height=2.5}
...
k1 <- kernel_umap_wrapper("HRD-Dup", c("FBI", "HRD-Del"), 0.015)
...
kernel_grid <- ggdraw() +
  ...
  draw_grob(get_legend(k1), x = 0.97, y = -0.15)

kernel_grid

ggsave("figures/320_cancer_cell_plotting_umap/320_umap_cancer_kernels.pdf", width = 7, height = 2.5)
ggsave("figures/320_cancer_cell_plotting_umap/320_umap_cancer_kernels.png", width = 7, height = 2.5)
...
plot_data_vio <- plot_data %>% 
  # sample_n(10000) %>% 
  select(umapharmony_1, umapharmony_2, cluster_label, consensus_signature, consensus_signature_short, tumor_supersite, tumor_megasite, all_of(gois), all_of(pois), sample_id) %>% 
  gather(key, value, -umapharmony_1, -umapharmony_2, -cluster_label, -consensus_signature, -consensus_signature_short, -tumor_supersite, -tumor_megasite, -sample_id) %>% 
...
violins_pw_mutsig_site <- boxplot_wrapper(consensus_signature, "Mutational\nsignature", pois_short, my_comparisons, "PROGENy score", c(-1,5), 10)
...
violins_pw_mutsig_pval_site <- boxplot_wrapper(consensus_signature, "Mutational\nsignature", pois_short, my_comparisons, "PROGENy score", c(-1,5), 10, "p.format")
...
```{r chunk_320_084, fig.width=5, fig.height=3}
...
```{r chunk_320_086, fig.width=8, fig.height=3}

violins_pw_cluster <- boxplot_wrapper(cluster_label, "Cluster", pois_short, ".all.", "Expression", c(-1,8), 10, mut_filter = levels(unique(scrna_meta_tbl$consensus_signature)), site_filter = names(clrs$tumor_supersite), facet_site = F)
...
violins_pw_cluster_pval <- boxplot_wrapper(cluster_label, "Cluster", pois_short, ".all.", "Expression", c(-1,8), 10, "p.format", mut_filter = levels(unique(scrna_meta_tbl$consensus_signature)), site_filter = names(clrs$tumor_supersite), facet_site = F)
...
```{r chunk_320_087, fig.width=6.5, fig.height=5}

violins_hla_mutsig_site <- boxplot_wrapper(consensus_signature, "Mutational\nsignature", gois[-7], my_comparisons, "Expression", c(0,6.1), 10)
...
violins_hla_mutsig_pval_site <- boxplot_wrapper(consensus_signature, "Mutational\nsignature", gois[-7], my_comparisons, "Expression", c(0,6.1), 10, "p.format")
...
```{r chunk_320_088, fig.width=6.5, fig.height=3.5}
...
```{r chunk_320_089, fig.width=6.5, fig.height=3.5}
...
```{r chunk_320_090, fig.width=10, fig.height=3.5}

violins_hla_cluster <- boxplot_wrapper(cluster_label, "Cluster", gois[-7], ".all.", "Expression", c(0,6.1), 10, mut_filter = levels(unique(scrna_meta_tbl$consensus_signature)), site_filter = names(clrs$tumor_supersite), facet_site = F)
...
violins_hla_cluster_pval <- boxplot_wrapper(cluster_label, "Cluster", gois[-7], ".all.", "Expression", c(0,6.1), 10, "p.format", mut_filter = levels(unique(scrna_meta_tbl$consensus_signature)), site_filter = names(clrs$tumor_supersite), facet_site = F)
...
violins_cd274_cluster <- boxplot_wrapper(cluster_label, "Cluster", "CD274", ".all.", "Expression", c(0,0.2), 10, site_filter = names(clrs$tumor_supersite), facet_site = F) + scale_y_continuous(breaks = c(0, 0.1, 0.2))
...
violins_cd274_cluster_pval <- boxplot_wrapper(cluster_label, "Cluster", "CD274", ".all.", "Expression", c(0,0.2), 10, "p.format", site_filter = names(clrs$tumor_supersite), facet_site = F) + scale_y_continuous(breaks = c(0, 0.1, 0.2))
