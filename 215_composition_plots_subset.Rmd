...
comp_list$Myeloid.super <- comp_list$Myeloid.super %>% 
  mutate(cell_type_dc = ifelse(str_detect(cluster_label, "DC"), "Dendritic.cell", "other"))

comp_list$B.super <- read_comp_wrapper("B.super", cluster_label)
comp_list$Endothelial.super <- read_comp_wrapper("Endothelial.super", cluster_label)
...
```{r chunk_215_106, fig.width=4, fig.height=2.5}

compute_composition_wrapper <- function(comp_data, clabel, clevel) {
  clevel <- enquo(clevel)
  comp_data %>%
    group_by(patient_id_short, consensus_signature, !!clevel) %>%
    summarise(n = sum(n)) %>%
    ...
    ungroup %>%
    mutate(rank = paste0(patient_id_short, "_", !!clevel)) %>%
    arrange(!!clevel, nrel) %>%
    mutate(rank = ordered(rank, levels = rank)) %>%
    mutate(!!clevel := ordered(!!clevel, levels = names(clrs[[as_label(clevel)]][[clabel]]))) %>%
    mutate(cell_type_super = clabel)
}

comp_norm_cc <- comp_list$Ovarian.cancer.super %>%
  compute_composition_wrapper("Ovarian.cancer.super", cluster_label)
comp_norm_mc <- comp_list$Myeloid.super %>%
  compute_composition_wrapper("Myeloid.super", cluster_label)
comp_norm_tc <- comp_list$T.super %>%
  compute_composition_wrapper("T.super", cluster_label_sub)
comp_norm_bc <- comp_list$B.super %>%
  compute_composition_wrapper("B.super", cluster_label)
comp_norm_fb <- comp_list$Fibroblast.super %>%
  compute_composition_wrapper("Fibroblast.super", cluster_label)
comp_norm_ec <- comp_list$Endothelial.super %>%
  compute_composition_wrapper("Endothelial.super", cluster_label)

# comp_norm_mois <- comp_list$Ovarian.cancer.super %>%
#   filter(consensus_signature %in% mois,
#          tumor_supersite != "Ascites") %>%
#   compute_composition_wrapper

ggplot(comp_norm_cc) +
  geom_bar(aes(rank, nrel*100, fill = consensus_signature), stat = "identity") +
  geom_point(aes(color = cluster_label), x = 21, y = 25, size = 8,
             data = distinct(comp_norm_cc, cluster_label)) +
...
ggsave("figures/215_composition_plots_subset/215_comp_plot_cluster_per_patient_Ovarian.cancer.super.pdf", width = 4, height = 2.5)
ggsave("figures/215_composition_plots_subset/215_comp_plot_cluster_per_patient_Ovarian.cancer.super.png", width = 4, height = 2.5)

```

```{r chunk_215_107, fig.width=4, fig.height=2.5}

# ggplot(comp_norm_mois) +
#   geom_bar(aes(cluster_label, nrel*100, fill = consensus_signature), 
...
#   remove_guides +
#   labs(y = "Patient composition across clusters [%]",
#        x = "Cluster")
# 
# ggsave("figures/215_composition_plots_subset/215_comp_plot_cluster_per_patient_Ovarian.cancer.super_stacked_barplot.pdf", width = 6, height = 2.5)
# ggsave("figures/215_composition_plots_subset/215_comp_plot_cluster_per_patient_Ovarian.cancer.super_stacked_barplot.png", width = 6, height = 2.5)

```

```{r chunk_215_108, fig.width=4, fig.height=2.5}

cluster_colors <- c(
  unlist(sapply(2:length(clrs$cluster_label), function(x) clrs$cluster_label[[x]])), 
  clrs$cluster_label_sub[[1]]
)

cluster_labels <- names(cluster_colors)

max_entropy <- entropy::entropy.empirical(rep(1/length(scrna_patients), length(scrna_patients)))

entropy_wrapper <- function(comp_norm_tbl, cts) {
  comp_norm_tbl %>%
    group_by(cluster_label) %>%
    summarise(entropy = entropy::entropy.empirical(nrel)) %>%
    arrange(entropy) %>% 
    ungroup() %>% 
    mutate(cluster_label = as.character(cluster_label),
           cell_type_super = cts,
           rel_entropy = max_entropy/entropy)
    
}

entropy_tbl <- bind_rows(
  entropy_wrapper(comp_norm_cc, "Ovarian cancer"),
  entropy_wrapper(rename(comp_norm_tc, cluster_label = cluster_label_sub), "T/NK"),
  entropy_wrapper(comp_norm_mc, "Myeloid"),
  entropy_wrapper(comp_norm_bc, "B.super"),
  entropy_wrapper(comp_norm_fb, "Fibroblast.super"),
  entropy_wrapper(comp_norm_ec, "Endothelial.super")
) %>% 
  mutate(cluster_label = ordered(cluster_label, levels = cluster_labels)) %>% 
  arrange(cell_type_super, cluster_label) %>% 
  group_by(cell_type_super) %>% 
  mutate(cluster_label_numeric = row_number(cell_type_super)) %>% 
  ungroup() %>% 
  mutate(inverse_entropy = 1/rel_entropy,
         patient_specific = inverse_entropy < 0.8)

entropy_tbl_sub <- entropy_tbl %>% 
  filter(cell_type_super %in% c("Ovarian cancer", "T/NK", "Myeloid")) %>% 
  mutate(cell_type_super = ordered(cell_type_super, c("Ovarian cancer", "T/NK", "Myeloid"))) %>% 
  mutate(anno_helper = "Cluster")

comp_wrapper <- function(comp_norm_tbl, cts) {
  comp_norm_tbl %>%
    mutate(cluster_label = as.character(cluster_label),
           cell_type_super = cts) %>% 
    select(-rank)
...
patient_comp <- bind_rows(
  comp_wrapper(comp_norm_cc, "Ovarian cancer"),
  comp_wrapper(rename(comp_norm_tc, cluster_label = cluster_label_sub), "T/NK"),
  comp_wrapper(comp_norm_mc, "Myeloid"),
) %>% 
  mutate(nrel = nrel*100) %>% 
  mutate(cluster_label = ordered(cluster_label, levels = cluster_labels)) %>% 
  arrange(cell_type_super, cluster_label) %>% 
  group_by(cell_type_super) %>% 
  mutate(cluster_label_numeric = deframe(select(entropy_tbl_sub, cluster_label, cluster_label_numeric))[as.character(cluster_label)]) %>% 
  ungroup() %>% 
  mutate(cell_type_super = ordered(cell_type_super, c("Ovarian cancer", "T/NK", "Myeloid")))

```

```{r chunk_215_109, fig.width=4, fig.height=2.5}

patient_bar <- ggplot(patient_comp) +
  geom_bar(aes(cluster_label_numeric, nrel, fill = patient_id_short),
           color = "black", size = 0.1,
           stat = "identity", position = "stack") +
  facet_grid(~cell_type_super, scales = "free_x", space = "free_x") +
  scale_y_continuous(expand = c(0, 0), breaks = c(0, 50, 100)) +
  scale_x_continuous(expand = c(0, 0)) +
  scale_fill_manual(values = clrs$patient_id_short) + 
  labs(y = "% cells") + 
  remove_xaxis +
  remove_guides

entropy_bar <- ggplot(entropy_tbl_sub, aes(cluster_label_numeric, inverse_entropy, fill = patient_specific)) +
  geom_bar(stat = "identity") +
  geom_hline(yintercept = 1, linetype = 1) +
  geom_hline(yintercept = 0.8, linetype = 2) +
  facet_grid(~cell_type_super, scales = "free_x", space = "free_x") +
  scale_fill_manual(values = c(`FALSE` = "grey80", `TRUE` = "red")) +
  scale_y_continuous(expand = c(0, 0), breaks = c(0, 0.5, 0.8, 1),
                     labels = c(0, 0.5, 0.8, "max: 1.0")) +
  scale_x_continuous(expand = c(0, 0)) +
...
entropy_tile <- ggplot(entropy_tbl_sub) +
  geom_tile(aes(cluster_label_numeric, anno_helper, fill = cluster_label)) +
  geom_point(aes(cluster_label_numeric, anno_helper), color = "white", alpha = 0.5, size = 5) +
  geom_text(aes(cluster_label_numeric, anno_helper, label = cluster_label_numeric)) +
  facet_grid(~cell_type_super, scales = "free_x", space = "free_x") +
...
plot_grid(patient_bar, entropy_tile, entropy_bar,
          ncol = 1, rel_heights = c(0.5, 0.1, 0.4), 
          align = "v", axis = "yx")

ggsave("figures/215_composition_plots_subset/215_cluster_entropy.pdf", width = 12, height = 5)
ggsave("figures/215_composition_plots_subset/215_cluster_entropy.png", width = 12, height = 5)
...
```{r chunk_215_140, fig.width=9, fig.height=9}

plist_dc <- default_comp_grid_list(
  comp_list$Myeloid.super, cell_type_dc, "Dendritic.cell",
  cluster_label, super_type = "Myeloid.super",
  mutsig_box = T, site_box = T, vec_plot = T,
  nmax = 5000, highlight = T)

plist_m1 <- default_comp_grid_list(
  comp_list$Myeloid.super, cluster_label, "M1.S100A8",
  cluster_label, super_type = "Myeloid.super",
  mutsig_box = T, site_box = T, vec_plot = T, 
  nmax = 5000, highlight = T)

plist_m2 <- default_comp_grid_list(
  comp_list$Myeloid.super, cluster_label, "M2.SELENOP",
  cluster_label, super_type = "Myeloid.super",
  mutsig_box = T, site_box = T, vec_plot = T, 
  nmax = 5000, highlight = T)

comp_grid_dc <- plot_grid(plotlist = plist_dc[-3], ncol = 1, align = "v", rel_heights = c(0.15, 0.15, 0.2, 0.5))

comp_grid_m1 <- plot_grid(plotlist = plist_m1[-3], ncol = 1, align = "v", rel_heights = c(0.15, 0.15, 0.2, 0.5))

comp_grid_m2 <- plot_grid(plotlist = plist_m2[-3], ncol = 1, align = "v", rel_heights = c(0.15, 0.15, 0.2, 0.5))

ggsave("figures/215_composition_plots_subset/215_comp_plot_Myeloid.super_DC.pdf", comp_grid_dc, width = 3, height = 9)
ggsave("figures/215_composition_plots_subset/215_comp_plot_Myeloid.super_DC.png", comp_grid_dc, width = 3, height = 9)

ggsave("figures/215_composition_plots_subset/215_comp_plot_Myeloid.super_M1.pdf", comp_grid_m1, width = 3, height = 9)
ggsave("figures/215_composition_plots_subset/215_comp_plot_Myeloid.super_M1.png", comp_grid_m1, width = 3, height = 9)

ggsave("figures/215_composition_plots_subset/215_comp_plot_Myeloid.super_M2.pdf", comp_grid_m2, width = 3, height = 9)
ggsave("figures/215_composition_plots_subset/215_comp_plot_Myeloid.super_M2.png", comp_grid_m2, width = 3, height = 9)

plot_grid(comp_grid_dc, comp_grid_m1, comp_grid_m2, nrow = 1)
ggsave("figures/215_composition_plots_subset/215_comp_plot_Myeloid.super_site_scrna.pdf", width = 9, height = 9)
ggsave("figures/215_composition_plots_subset/215_comp_plot_Myeloid.super_site_scrna.png", width = 9, height = 9)

```

## PCA based on cluster compositions

```{r chunk_215_150, fig.width=15, fig.height=4}

## PCA input matrix
comp_mat <- bind_rows(
  comp_list$Myeloid.super %>%
    select(sample_id, cluster_label, nrel) %>%
    spread(sample_id, nrel),
  comp_list$T.super %>%
    select(sample_id, cluster_label = cluster_label_sub, nrel) %>%
    spread(sample_id, nrel),
  comp_list$B.super %>%
    select(sample_id, cluster_label, nrel) %>%
    spread(sample_id, nrel)
...
pca_meta_data <- bind_rows(comp_list$Myeloid.super, comp_list$T.super, comp_list$B.super) %>% 
  distinct(sample_id, tumor_supersite, tumor_megasite, consensus_signature)

comp_mat[is.na(comp_mat)] <- 0

pca_result <- prcomp(comp_mat)

pca_features <- summary(pca_result)$importance %>% 
  as.data.frame() %>% 
  as_tibble(rownames = "feature") %>% 
  gather(PC, value, -feature)

pca_pcs <- pca_result$x %>% 
  as.data.frame %>% 
  as_tibble(rownames = "sample_id") %>% 
  select(sample_id, PC1, PC2) %>% 
  left_join(pca_meta_data, by = "sample_id")

pca_rot <- pca_result$rotation %>% 
  as.data.frame %>% 
  as_tibble(rownames = "cluster_label") %>% 
  select(cluster_label, PC1, PC2)

pc1_loadings <- pca_rot %>% 
  arrange(-abs(PC1)) %>% 
  slice(1:10) %>% 
  select(cluster_label_pc1 = cluster_label, PC1)

pc2_loadings <- pca_rot %>% 
  arrange(-abs(PC2)) %>% 
  slice(1:10) %>% 
  select(cluster_label_pc2 = cluster_label, PC2)

bind_cols(pc1_loadings, pc2_loadings)

plot_pcs <- function(pcx, pcy) {
  
  pcx <- enquo(pcx)
  pcy <- enquo(pcy)
  
  p1 <- ggplot(pca_pcs) +
    geom_point(aes(!!pcx, !!pcy, color = tumor_supersite)) + 
    scale_color_manual(values = clrs$tumor_supersite[names(clrs$tumor_supersite) %in% pca_pcs$tumor_supersite]) +
    theme(aspect.ratio = 1)
  
  p2 <- ggplot(pca_pcs) +
    geom_point(aes(!!pcx, !!pcy, shape = tumor_megasite)) +
    scale_shape_manual(values = shps$tumor_megasite[names(shps$tumor_megasite) %in% pca_pcs$tumor_megasite]) +
    theme(aspect.ratio = 1)

  p3 <- ggplot(pca_pcs) +
    geom_point(aes(!!pcx, !!pcy, color = consensus_signature)) +
    scale_color_manual(values = clrs$consensus_signature[names(clrs$consensus_signature) %in% pca_pcs$consensus_signature]) +
    theme(aspect.ratio = 1)
...
ggsave("figures/215_composition_plots_subset/215_comp_PCA.pdf", width = 15, height = 4)
ggsave("figures/215_composition_plots_subset/215_comp_PCA.png", width = 15, height = 4)
