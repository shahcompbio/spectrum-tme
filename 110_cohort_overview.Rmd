...
```{r}
# Load database
db <- readr::read_rds("/work/shah/vazquezi/projects/spectrum/results/db/v1/database/outputs/tme_db/db/SPECTRUM.rds")
```
...
inventory <- plyr::join_all(
  list(db$sequencing_scrna, db$sequencing_bulk_dna, db$sequencing_myriad, db$sequencing_msk_impact_custom, db$he_slide, db$mpif_slide),
...
inventory <- inventory %>% mutate(
    tumor_type = ordered(tumor_type, levels = names(colors$tumor_type)),
    tumor_supersite = ordered(tumor_supersite, levels = names(colors$tumor_supersite)),
    therapy = ordered(therapy, levels = names(colors$therapy))
    )
...
```{r}
mylist <- inventory %>% 
  # dplyr::recode(technique, scrna = `scRNA`, bulk_dna = `Bulk WGS`, myriad = `Myriad`, impact = `MSK-IMPACT`, he_slide = `H&E`, mpif_slide = `mpIF`) %>%
  split(.$technique)
...
data <- data %>% 
  ...
  mutate(
    ...
    consensus_signature = ordered(consensus_signature, levels = names(colors$consensus_signature)),
    consensus_signature_short = ordered(consensus_signature_short, levels = names(colors$consensus_signature_short))
    ) %>%
...
top_df <- db$mutational_signatures %>%
  ...
  filter(patient_id %in% c("002")) %>%
  arrange(match(patient_id, colnames(oncoprint_main_mat))) %>%
  mutate(consensus_signature = ordered(consensus_signature, levels = names(colors$consensus_signature))) %>%
...
colours <-
  list(
    ...
    "Surgery" = colors$chemo_intent[names(colors$chemo_intent)!="Salvage"],
...
pdf("figures/110_cohort_overview/110_oncoprint_spectrum_scrna_patients.pdf", width = 13.5, height = 4.4)
...
pdf("figures/110_cohort_overview/110_oncoprint_spectrum_all_patients.pdf", width = 13.5, height = 4.4)
print(oncoprint_supp_ht)
dev.off()
...
data <- inventory %>%
  ...
  dplyr::select(-c("H&E_Ascites","H&E_Other","mpIF_Ascites"))
...
pdf("figures/110_cohort_overview/110_composite.pdf", width = 10, height = 4)
composite
dev.off()
