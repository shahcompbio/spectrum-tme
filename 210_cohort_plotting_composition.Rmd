...
for(i in 1:length(cell_types_immune)){
  ...
  plist <- default_comp_grid_list(
    filter(comp_tbl_sample, sort_short_x == "CD45+", consensus_signature %in% sois), 
...
for(i in 1:length(tcga_subtypes)){
  ...
  plist <- default_comp_grid_list(
    filter(comp_tbl_consOV, sort_short_x == "CD45+", consensus_signature %in% sois), 
...
```{r chunk_210_310, fig.width=16, fig.height=8}
...
```{r chunk_210_330, fig.width=20, fig.height=16}
