...
consOV_tbl <- read_tsv("/work/shah/uhlitzf/data/SPECTRUM/freeze/v7/consensusOV/SPECTRUM_freeze_v7_consensusOV.tsv") %>%
  ...
  dplyr::select(cell_id, consensusOV)
...
comp_tbl_sample <- seu_tbl_full %>%
  filter(patient_id %in% included_patients, therapy == "pre-Rx", cell_type != "Other") %>%
...
# pdf("_fig/210_cohort/210_comp_full.pdf", width = 3.5, height = 12)
...
# names(clrs$cell_type) <- str_replace_all(names(clrs$cell_type), "Ovarian", "Ov")
# 
# mpif_pixel <- read_tsv("/work/shah/vazquezi/projects/spectrum/results/mpif/v10/integrate/outputs/cohort_merge/patient/SPECTRUM/all/detection.tsv") %>%
#   dplyr::select(cell_id, compartment = Parent)
# 
# mpif_cell_type <- read_tsv("/work/shah/vazquezi/projects/spectrum/results/mpif/v10/integrate/outputs/cohort_merge/patient/SPECTRUM/all/cell_type_manual.tsv") %>%
...
# )
# 
# # write_tsv(mpif_cell_type_expanded, "/work/shah/vazquezi/projects/spectrum/papers/spectrum-tme/notebooks/cell_type_manual_expanded.tsv")
# 
...
mpif_cell_state <- read_tsv("/work/shah/vazquezi/projects/spectrum/results/mpif/v10/cell-type/outputs/phenotype_summary_merged/SPECTRUM.tsv")

mpif_cell_state <- mpif_cell_state %>%
# mpif_cell_state_expanded <- read_tsv("/work/shah/uhlitzf/data/SPECTRUM/mpIF/cell_type_manual_expanded.tsv") %>%
  tidyr::separate(Image, c("patient_id","tumor_subsite","markers",NA,"fov_id"), sep = "_", remove = FALSE) %>%
  mutate(fov_id = paste0(patient_id, "_", tumor_subsite, "_", fov_id),
         slide_id = paste0(patient_id, "_", tumor_subsite)) %>%
  dplyr::select(-c('patient_id','tumor_subsite')) %>%
  left_join(mpif_slide_meta_tbl, by = c("slide_id")) %>%
  left_join(db$mpif_fov %>% dplyr::select(c('isabl_id','fov_status')), by = c("Image" = "isabl_id")) %>%
  # dplyr::select(cell_id, slide_id, fov_id, compartment, cell_type, cell_state, contains("state")) %>%
  # left_join(dplyr::select(mpif_meta_tbl, slide_id, patient_id, patient_id_short, tumor_supersite, tumor_subsite, therapy, sample_id, consensus_signature, tumor_megasite), by = "slide_id") %>%
  # na.omit() %>%
  mutate(cell_phenotype_mpif = phenotype, cell_type_mpif = cell_type) %>%
  ...
  filter(Parent %in% c("Tumor", "Stroma"))# %>%
  # # Remove excluded FOVs
  # filter(fov_status != "exclude")

## cell state composition fov lvl
mpif_cell_state_n <- mpif_cell_state %>%
  group_by(patient_id_short, tumor_supersite, tumor_subsite, tumor_megasite, therapy, sample_id, fov_id, consensus_signature, cell_phenotype_mpif, cell_type_mpif) %>%
...
mpif_cell_state_n_slide <- mpif_cell_state %>%
  group_by(patient_id_short, tumor_supersite, tumor_subsite, tumor_megasite, therapy, sample_id, consensus_signature, cell_phenotype_mpif, cell_type_mpif) %>%
  summarize(n = sum(count)) %>%
...
mpif_cell_state_n_slide_compartment <- mpif_cell_state %>%
  group_by(patient_id_short, tumor_supersite, tumor_subsite, tumor_megasite, therapy, sample_id, consensus_signature, sort_short_x, cell_phenotype_mpif, cell_type_mpif) %>%
  summarize(n = sum(count)) %>%
...
mpif_cell_type_n_slide_compartment <- mpif_cell_state %>%
  group_by(patient_id_short, tumor_supersite, tumor_subsite, tumor_megasite, therapy, sample_id, consensus_signature, sort_short_x, cell_type_mpif) %>%
  summarize(n = sum(count)) %>%
  group_by(patient_id_short, tumor_supersite, tumor_subsite, tumor_megasite, therapy, sample_id, consensus_signature, sort_short_x) %>%
  mutate(nrel = n/sum(n)*100) %>%
  ungroup() %>%
...
cell_types_mpif <- c("Ovarian.cancer.cell", "Monocyte", "CD8.T.cell", "Other", "Unknown")
...
for(i in 1:length(cell_types_mpif)){
  ...
  plist_t <- default_comp_grid_list(
    ...
    cell_type_mpif, cell_types_mpif[i], cell_phenotype_mpif, mutsig_box = F, 
    nmax = 250000)
...
for(i in 1:length(cell_types_mpif)){
  ...
  plist_t <- default_comp_grid_list(
    filter(mpif_cell_state_n_slide_compartment, sort_short_x == "Tumor"), 
    cell_type_mpif, cell_types_mpif[i], cell_phenotype_mpif, site_box = F, #vec_plot = F,
    nmax = 250000)
...
cell_states_mpif <- names(clrs$cell_phenotype_mpif)
...
for(i in 1:length(cell_states_mpif)){
  ...
  plist_t <- default_comp_grid_list(
    filter(mpif_cell_state_n_slide_compartment, sort_short_x == "Tumor"), 
    cell_phenotype_mpif, cell_states_mpif[i], cell_phenotype_mpif, mutsig_box = F, #vec_plot = F,
    nmax = 250000)
...
markers_pos_frac_scrna_celltype <- seu_tbl_full %>%
  # dplyr::select(cell_id, sample) %>%
  dplyr::select(cell_id, sample, tumor_supersite, consensus_signature, patient_id_short, sort_short_x) %>%
  left_join(dplyr::select(scrna_markers, cell_id, cell_type_sc, cell_state), by = "cell_id") %>%
  na.omit() %>%
  ...
  mutate(nrel = n/sum(n)*100) %>%
  dplyr::select(sample, cell_type_sc, n, nrel, everything()) %>%
  ungroup
...
markers_pos_frac_scrna_cellstate <- seu_tbl_full %>%
  ...
  group_by(sample, cell_type_sc, tumor_supersite, consensus_signature, patient_id_short, sort_short_x) %>%
  mutate(nrel = n/sum(n)*100) %>%
  dplyr::select(sample, cell_type_sc, cell_state, n, nrel, everything()) %>%
  ungroup

markers_pos_frac_scrna_gene <- seu_tbl_full %>%
  # dplyr::select(cell_id, sample) %>%
  dplyr::select(cell_id, sample, cell_type, tumor_supersite, consensus_signature, patient_id_short, sort_short_x) %>%
  left_join(dplyr::select(scrna_markers, -sample, -cell_type), by = "cell_id") %>%
  na.omit() %>%
  ...
  dplyr::select(sample, gene, n, nrel, everything())
...
#   dplyr::select(sample = sample_id, cell_type_sc = cell_type_mpif,
...
#   group_by(sample_id, sort_short_x, cell_type_mpif) %>%
...
```{r chunk_250, fig.width=6, fig.height=3}
...
```{r chunk_260, fig.width=7, fig.height=12, results="asis"}

cell_types_hne <- c("Lymphocyte", "Other")
...
```{r chunk_270, fig.width=7, fig.height=12, results="asis"}
...
```{r chunk_280, fig.width=12, fig.height=6}
...
plist5 <- default_comp_grid_list(
  ...
  cell_type_mpif, "CD8.T.cell", cell_type_mpif, nmax = 250000)
plist5$empty <- ggdraw()
...
pcomp3 <- plot_grid(plotlist = plist5[-3], ncol = 1, align = "v",
                    rel_heights = c(0.13, 0.13, 0.16, 0.4, 0.18))
...
mpif_cell_type_legend <- get_legend(plot_comp_bar(
  ...
  sample_id, nrel, cell_type_mpif, facet = "sort_short_x") +
  scale_color_manual(values = c(Adnexa = "#ff0000", Other = "#56B4E9"), labels = c("Enriched", "Depleted", "")) +
...
```{r chunk_290, fig.width=14, fig.height=8}
...
markers_pos_scrna_mpif_state_cd45p <- mpif_cell_state_n_slide_compartment %>%
  ...
  left_join(markers_pos_frac_scrna_cellstate, by = c("sample", "cell_type_sc", "cell_state")) %>%
  filter(sort_short_x == "CD45+",
         cell_phenotype_mpif %in% c("CD8+PD1+TOX+", "CD68+PDL1+"),
         compartment %in% c("Stroma", "Tumor"))

markers_pos_scrna_mpif_state_cd45n <- mpif_cell_state_n_slide_compartment %>%
  group_by(sample_id, sort_short_x, cell_type) %>%
  mutate(nrel_state = n/sum(n)*100) %>%
  dplyr::select(sample = sample_id, cell_type_sc = cell_type,
         compartment = sort_short_x, n_mpif = n, 
...
```{r chunk_300, fig.width=19, fig.height=11}
...
ggsave(filename = "figures/210_cohort_cell_type_composition/210_comp_grid_site_mutsig.pdf", comp_grid_full,
       width = 19, height = 11)

ggsave(filename = "figures/210_cohort_cell_type_composition/210_comp_grid_site_mutsig.png", comp_grid_full,
       width = 19, height = 11)

```

```{r chunk_310, fig.width=16, fig.height=8}
...
```{r chunk_320, fig.width=14, fig.height=8}
...
```{r chunk_330, fig.width=20, fig.height=16}
...
mpif_lvls <- mpif_cell_type_n_slide_compartment %>% 
  rank_by(cell_type_mpif, "CD8.T.cell", cell_type_mpif) %>% 
  distinct(sample_id, sample_id_lvl)

plist_mpif <- default_comp_grid_list(
  filter(mpif_cell_type_n_slide_compartment), 
  cell_type_mpif, "CD8.T.cell", cell_type_mpif, facet = patient_id_short, nmax = 500000,
  site_box = F, vec_plot = F, mutsig_box = F, site_tiles = T, mutsig_tiles = T)

pbar_mpif_tumor <- mpif_cell_type_n_slide_compartment %>% 
  filter(sort_short_x == "Tumor") %>% 
  left_join(mpif_lvls, by = "sample_id") %>% 
  mutate(cell_type_mpif = ordered(cell_type_mpif, levels = unique(c("CD8.T.cell", names(clrs$cell_type_mpif))))) %>% 
  plot_comp_bar(sample_id_lvl, nrel, cell_type_mpif, nmax = 500000, 
                facet = patient_id_short) +
  guides(fill = F) +
  labs(y = "% cells\n(Tumor)")
...
pgrid_supplement_full <- plot_grid(
  ...
  ggdraw(),
  plot_grid(pgrid_supplement3, ggdraw(), nrow = 1, rel_widths = c(0.9, 0.1)), 
  ggdraw(),
...
ggsave(filename = "figures/210_cohort_cell_type_composition/210_comp_grid_per_patient.pdf", 
       pgrid_supplement_full, width = 20, height = 16)
