...
cois <- c("T.super", "Myeloid.super", "Fibroblast.super", "Ovarian.cancer.super", "B.super", "Endothelial.super")
cois_cd45p <- c("T.super", "Myeloid.super", "B.super")
cois_cd45n <- c("Fibroblast.super", "Ovarian.cancer.super", "Endothelial.super")
...
## scRNA composition PCA 

```{r chunk_210_140, fig.width=6, fig.height=3}

comp_list$T.super$cluster_label <- comp_list$T.super$cluster_label_sub

transpose_comp <- . %>% 
  # mutate(sample = str_remove_all(sample, "CD45P_|CD45N_|UNSORTED_")) %>% 
  filter(!str_detect(sample, "UNSORTED")) %>% 
  select(sample, nrel, cluster_label) %>% 
  spread(sample, nrel)

comp_list_transposed_cd45p <- lapply(comp_list[cois_cd45p], transpose_comp) %>% 
  bind_rows

comp_list_transposed_cd45n <- lapply(comp_list[cois_cd45n], transpose_comp) %>% 
  bind_rows

exclude_samples_cd45p <- apply(comp_list_transposed_cd45p, 2, is.na) %>% 
  apply(2, any)

exclude_samples_cd45n <- apply(comp_list_transposed_cd45n, 2, is.na) %>% 
  apply(2, any)
...
comp_matrix_cd45p <- comp_list_transposed_cd45p[,!exclude_samples_cd45p] %>% 
  make_matrix_comp
comp_matrix_cd45n <- comp_list_transposed_cd45n[,!exclude_samples_cd45n] %>% 
  make_matrix_comp

comp_pca_cd45p <- prcomp(comp_matrix_cd45p)
comp_pca_cd45n <- prcomp(comp_matrix_cd45n)

comp_pca_tbl_cd45p <- comp_pca_cd45p$x[,1:5] %>% 
  as_tibble(rownames = "sample") %>% 
  left_join(scrna_meta_tbl, by = "sample")

comp_pca_tbl_cd45n <- comp_pca_cd45n$x[,1:5] %>% 
  as_tibble(rownames = "sample") %>% 
  left_join(scrna_meta_tbl, by = "sample")

```

```{r chunk_210_150, fig.width=6, fig.height=3}

comp_pca_cd45p$rotation[,1:2] %>% 
...
```{r chunk_210_160, fig.width=9, fig.height=5}

remove_axis_lines <- theme(axis.text = element_blank(),
                           axis.line = element_blank(),
                           axis.ticks = element_blank())

p1 <- ggplot(comp_pca_tbl_cd45p, aes(PC1, PC2, color = tumor_supersite)) + 
  geom_point() +
  scale_color_manual(values = clrs$tumor_supersite) +
  remove_axis_lines

p2 <- ggplot(comp_pca_tbl_cd45n, aes(PC1, PC2, color = tumor_supersite)) + 
  geom_point() +
  scale_color_manual(values = clrs$tumor_supersite) +
  remove_axis_lines

p3 <- ggplot(comp_pca_tbl_cd45p, aes(PC1, PC2, color = consensus_signature)) + 
  geom_point() +
  scale_color_manual(values = clrs$consensus_signature) +
  remove_axis_lines

p4 <- ggplot(comp_pca_tbl_cd45n, aes(PC1, PC2, color = consensus_signature)) + 
  geom_point() +
  scale_color_manual(values = clrs$consensus_signature) +
  remove_axis_lines
...
```{r chunk_210_240, fig.width=6, fig.height=3, eval=F}
...
```{r chunk_210_250, fig.width=7, fig.height=12, results="asis", eval=F}
...
```{r chunk_210_260, fig.width=7, fig.height=12, results="asis", eval=F}
...
```{r chunk_210_270, fig.width=7, fig.height=12, results="asis", eval=F}
...
```{r chunk_210_280, fig.width=7, fig.height=12, results="asis", eval=F}
...
```{r chunk_210_290, fig.width=6, fig.height=3, eval=F}
...
```{r chunk_210_300, fig.width=16, fig.height=8, eval=F}
...
```{r chunk_210_320, fig.width=12, fig.height=12, eval=F}
...
```{r chunk_210_330, fig.width=16, fig.height=8, eval=F}
...
```{r chunk_210_340, fig.width=12, fig.height=6, eval=F}
...
```{r chunk_210_350, fig.width=6, fig.height=3, eval=F}
...
```{r chunk_210_360, fig.width=7, fig.height=12, results="asis", eval=F}
...
```{r chunk_210_370, fig.width=7, fig.height=12, results="asis", eval=F}
...
```{r chunk_210_380, fig.width=12, fig.height=6, eval=F}
...
```{r chunk_210_390, fig.width=14, fig.height=8, eval=F}
...
```{r chunk_210_400, fig.width=17, fig.height=10, eval=F}
...
```{r chunk_210_410, fig.width=17, fig.height=11, eval=F}
...
```{r chunk_210_315, fig.width=16, fig.height=8, eval=F}
...
```{r chunk_210_420, fig.width=14, fig.height=8, eval=F}
...
```{r chunk_210_430, fig.width=22, fig.height=18, eval=F}
